...
| `occupation-wages` | Occupation Skills and Wages Map, O*NET-SOC with top-8 skills and BLS OEWS May 2025 median wage (staged, not listed for sale yet) | 1,016 | labor | [CSV](data/occupation-wages/sample.csv) / [JSON](data/occupation-wages/sample.json) | [preview page](datasets/occupation-wages.html) |

Every dataset ships with a sample in this repo (9-25 rows per table, small sets complete). Free datasets also carry their complete files in-repo, and four big packs (NFL games, HUD FMR FY2026, HUD FMR FY2027, Airbnb 6-cities) download free from Zenodo. Total data volume: about 18 MB.
...
- Staged product previews with free samples and no buy link yet: `datasets/occupation-wages.html` (1,016 occupations x 19 columns), `agents/agent-skills-vol1.html` (10 `SKILL.md` files, two printed in full), `printables/planner-2027.html` (92-page dated planner)
...
License: CC BY 4.0 for samples, except OpenStreetMap-derived samples (ODbL 1.0, © OpenStreetMap contributors). Full data licensed per purchase. Upstream sources credited per dataset page: nflverse (CC BY 4.0), HUD (public domain), Inside Airbnb (CC BY 4.0), Eurostat (CC BY 4.0), World Bank Open Data (CC BY 4.0), OpenStreetMap contributors (ODbL 1.0), superteam.fun (harvested API data), O*NET 31.0 Database (CC BY 4.0, USDOL/ETA; the crosswalk page carries the required attribution paragraph and the list of modifications). If you republish samples, credit "Data Vault (jayjex.github.io/data-vault)" plus the upstream source and its license (CC BY 4.0, or ODbL 1.0 for OpenStreetMap data).
